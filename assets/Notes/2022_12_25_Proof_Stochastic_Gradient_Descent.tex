...
\title{Stochastic gradient descent proof}
...
\paragraph{{\color{red}Problem setup.}} We are interested in solving
the problem\marginnote{\textbf{Notation}
...
\begin{assumption}[Structure of $\mathcal{P}$]\label{assum:problem}
...
\paragraph{{\color{red}Stochastic gradient descent.}} The stochastic
gradient descent (SGD) algorithm to solve \eqref{eq:problem} is described
by Algorithm \ref{alg:sgd}, where we make the following assumption
regarding the nature of the oracle.

\begin{algorithm}[h]
\textbf{\small{}input: }{\small{}$f$, $C$, iteration limit $K$}{\small\par}

{\small{}\hrulefill}{\small\par}

\textbf{\small{}algorithm:}{\small\par}
...
\textbf{\small{}3. return}{\small{} }\textbf{\small{}$x_{K}$}\caption{SGD to solve \eqref{eq:problem}}
\label{alg:sgd}
\end{algorithm}
...
\paragraph{{\color{red}Convergence analysis.}} First, note that,
for all $k\geq0$:
